...
from resnet_2d import resnet18_feature_extractor, resnet50_feature_extractor


class RBG_Extractor(nn.Module):
    '''Simple average RGB over time for further computing RGB diff'''
    def __init__(self, pretrained=True, num_seq=2, seq_len=5, pred_step=1):
        super(RBG_Extractor, self).__init__()
        torch.cuda.manual_seed(233)
        print('Using average RGB over time in order to further computing RGB diff')
    ...
    def forward(self, block):
        ...
        block = torch.mean(block, dim=2).unsqueeze(dim=2) # average over temporal, keep spatial size
        return [block[:, 1], block[:, 0]] # each of shape [B, pred_step=1*seq_len after pool=1, D, last_size, last_size]
    
    
class ResNet50_BeforePool_Feature_Extractor(nn.Module):
    '''ResNet50 extract feature at BeforePool'''
    def __init__(self, pretrained=True, num_seq=2, seq_len=5, pred_step=1):
        super(ResNet50_BeforePool_Feature_Extractor, self).__init__()
        torch.cuda.manual_seed(233)
        print('Using ResNet50 model to extract feature at BeforePool')
        self.backbone = resnet50_feature_extractor(pretrained=True, progress=True)
    ...
    def forward(self, block):
        ...
        del block
        feature = feature.reshape(B, N, SL, feature.shape[-3], feature.shape[-2], feature.shape[-1])
        feature = torch.mean(feature, dim=2).unsqueeze(dim=2) # average over temporal, keep spatial size
        #ipdb.set_trace()
        return [feature[:, 1], feature[:, 0]]
    
    
class ResNet18_BeforePool_Feature_Extractor(nn.Module):
    '''ResNet18 extract feature at BeforePool'''
    def __init__(self, pretrained=True, num_seq=2, seq_len=5, pred_step=1):
        super(ResNet18_BeforePool_Feature_Extractor, self).__init__()
        torch.cuda.manual_seed(233)
        print('Using ResNet18 model to extract feature at BeforePool')
        self.backbone = resnet18_feature_extractor(pretrained=True, progress=True)
    ...
    def forward(self, block):
        ...
        feature = torch.mean(feature, dim=2).unsqueeze(dim=2) # average over temporal, keep spatial size
        #ipdb.set_trace()
        return [feature[:, 1], feature[:, 0]] # each of shape [B, pred_step=1*seq_len after pool=1, D, last_size, last_size] [B, 1, 512, 7, 7]
    
    
class DPC_RNN_Infer_Pred_Error(nn.Module):
    '''DPC with RNN'''
    ...
    def forward(self, block):
        ...
        pred = pred.view(B, self.pred_step, self.param['feature_size'], self.last_size, self.last_size)
        feature_inf = feature_inf.view(B, self.pred_step, self.param['feature_size'], self.last_size, self.last_size)
        del hidden
        
        return [feature_inf, pred] # each of shape [B, pred_step, D, last_size, last_size]
